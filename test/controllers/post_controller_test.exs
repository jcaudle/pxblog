defmodule Pxblog.PostControllerTest do
  ...
  test "redirects when trying to edit a post for a different user", %{conn: conn, user: user} do
    other_user = User.changeset(%User{}, %{email: "test2@example.com", username: "test2", password: "test", password_confirmation: "test"})
                 |> Repo.insert!
    post = build_post(user)
  ...
  defp create_user do
    User.changeset(%User{}, %{email: "test@example.com", username: "test", password: "test", password_confirmation: "test"})
    |> Repo.insert
  end
